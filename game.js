...
function checkForCompleteSquares(newSelectedLine)
{
	...
	var output = [];
	var Rowh, Colh, Rowv, Colv, ColvLeft, ColvRight, RowhTop, RowhBottom, ColhRight;
	var IndexLeftVertical, IndexRightVertical, IndexBottomHorizontal, IndexUpHorizontal, IndexTopHorizontal;
...
function dot(x, y) {
   dot.x = x;
   dot.y = y;
   dot.OutputShape = new Kinetic.Circle({
   	x: dot.x,
   	y: dot.y,
   	radius: 10,
   	fill: "#DDDDDD"
   });
   dot.OutputShape.on("mouseover", function() {
   	this.fill("#AAAAAA");
   	gLayer.draw();
   });
   dot.OutputShape.on("mouseout", function() {
   	this.fill("#DDDDDD");
   	gLayer.draw();
   });
   dot.draw = function() {
...
function line(x1, y1, x2, y2) {
   ...
   line.OutputShape = new Kinetic.Line({
   ...
   //Hover effect
   line.OutputShape.on("mouseover", function() {
   	line.stroke(currentPlayer.color);
   	gLayer.draw();
   });
   line.OutputShape.on("mouseout", function() {
   	line.stroke(line.color);
   	gLayer.draw();
   });
   //Click event
   line.OutputShape.on("click", function() {
   	if (!line.capturedLine)
   	{
	   	line.capturedLine = true;
	   	line.color = currentPlayer.color;
	   	var index = gLines.indexOf(this);
